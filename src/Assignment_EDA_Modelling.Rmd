...
seasonal_naive_daily <- full_dataset %>%
...
#### 2. Split training set into train and validation (28 days)

```{r}

# Determine the horizon and thus the cutoff value
forecast_horizon <- 28
cutoff <- max(seasonal_naive_daily$date) - days(forecast_horizon)

# Split data into train and validation
ts_train <- seasonal_naive_daily %>% filter(date <= cutoff)
ts_validation  <- seasonal_naive_daily %>% filter(date > cutoff)
...
#### 4. Forecast and report the results
...
autoplot(ts_train, sales) + autolayer(fc_baseline)
```

### 5. Diagnostics (if needed)

```{r}
# TODO: Jeremi's Magic Here.
```
