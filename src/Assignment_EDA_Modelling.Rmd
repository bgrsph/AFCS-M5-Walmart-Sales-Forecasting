...
author:
  - "Bugra Sipahioglu (14318334)"
  - "Christos Georghiou (16072766)"
  - "Despoina Delipalla (16324897)"
  - "Jeremi Wasilewski (14271028)"
...
validation_data <- read_csv("../data/sales_test_validation_afcs2025.csv")
evaluation_data <- read_csv("../data/sales_test_evaluation_afcs_2025.csv")
...
Calendard contains information about the dates the products are sold.
...
sales_nc_validation <- validation_data %>%
  separate(
    id,
    into = c("item_id", "dept_id", "cat_id", "state_id", "state_store_id", "dataset_name"),
    sep = "_"
  ...
  select(-state_id, -state_store_id)
sales_validation <- sales_nc_validation %>%
  pivot_longer(
    cols = starts_with("d_"),
    names_to  = "day_id",
    names_transform = list(day_id = ~ sub("^d_", "id_", .)),
...
sales_nc_evaluation <- evaluation_data %>%
  separate(
    id,
    into = c("item_id", "dept_id", "cat_id", "state_id", "state_store_id", "dataset_name"),
    sep = "_"
  ...
  select(-state_id, -state_store_id)
sales_evaluation <- sales_nc_evaluation %>%
  pivot_longer(
    cols = starts_with("d_"),
    names_to  = "day_id",
    names_transform = list(day_id = ~ sub("^d_", "id_", .)),
...
calendar_sales_validation <- sales_validation%>%
  left_join(calendar, by = "day_id")

head(calendar_sales_validation)

# Repeat for evaluation
calendar_sales_evaluation <- sales_evaluation%>%
  left_join(calendar, by = "day_id")

head(calendar_sales_evaluation)
```
...
# Repeat for validation
full_dataset_validation <- calendar_sales_validation %>%
  left_join(
    sell_prices,
    by = c("store_id", "item_id", "wm_yr_wk")
  )
...
# Repeat for evaluation
full_dataset_evaluation <- calendar_sales_evaluation %>%
  left_join(
    sell_prices,
    by = c("store_id", "item_id", "wm_yr_wk")
  )
...
```{r}
#str(full_dataset)
#str(full_dataset_validation)
str(full_dataset_evaluation)
...
```{r}
# Prepare train, validation and evaluation (test) datasets
ts_train <- full_dataset %>%
...
ts_validation <- full_dataset_validation %>%
  mutate(date = as.Date(date)) %>%
  group_by(date) %>%
  summarise(sales = sum(units_sold, na.rm = TRUE), .groups = "drop") %>%
  arrange(date) %>%
...
ts_evaluation <- full_dataset_evaluation %>%
  mutate(date = as.Date(date)) %>%
  group_by(date) %>%
  summarise(sales = sum(units_sold, na.rm = TRUE), .groups = "drop") %>%
  arrange(date) %>%
...
Check out whether training, validation and evaluation datasets are set
correctly.

```{r}
# Print out the range values to check the continuity of the dates
range(ts_train$date)
range(ts_validation$date)
range(ts_evaluation$date)
```
...
```{r}
# Fit the baseline models using training data
models <- ts_train %>%
  model(
    Mean          = MEAN(sales),
    Naive         = NAIVE(sales),
...
### 3.1 Forecast and report the results for validation dataset

```{r}
# Forecast and report 80% & 90% prediction intervals
fc_baseline_validation <- models %>% forecast(h = forecast_horizon, level = c(80, 95))

# Calculate accuracy for the evaluation predictions
accuracy_baseline_validation <- accuracy(fc_baseline_validation, ts_validation)
accuracy_baseline_validation

# Plot the data and predictions
autoplot(ts_train, sales) +
  autolayer(fc_baseline_validation) +
  labs(title = "Baseline Forecasts (28 days)", x = "Date", y = "Sales")
```

### 3.2 Forecast and report the results for evaluation dataset

```{r}
# Forecast and report 80% & 90% prediction intervals
fc_baseline_evaluation <- models %>% forecast(new_data = ts_evaluation, level = c(80, 95))

# Calculate accuracy for the evaluation predictions
accuracy_baseline_evaluation <- accuracy(fc_baseline_evaluation, ts_evaluation)
accuracy_baseline_evaluation

# Plot the data and predictions
autoplot(ts_train, sales) +
  autolayer(fc_baseline_evaluation) +
  labs(title = "Baseline Forecasts (28 days)", x = "Date", y = "Sales")
```
...
models |> 
  select(Mean) |> 
  augment() |> features(.innov, ljung_box, lag = 28, dof = 0)
...
No fundamental pre-processing is needed as the datasets that are used in
seasonal naive are sufficient for ARIMA modelling.
...
```{r}
# Forecast and report 80% & 90% prediction intervals
fc_arima_validation <- fit_arima %>%
  forecast(h = forecast_horizon, level = c(80, 95))


# Calculate accuracy for the validation predictions
accuracy_arima_validation <- accuracy(fc_arima_validation, ts_validation)
accuracy_arima_validation

# Plot the predictions
autoplot(ts_train, sales) +
  autolayer(fc_arima_validation) +
  labs(title = "ARIMA Forecast (28 days)", x = "Date", y = "Sales")
```

### 3.2 Forecast and report the results for evaluation dataset

Here, since ARIMA model must start one step beyond the end of the
training data and the evaluation datasets starts 28 days after training,
forecasting is done for 56 days.

```{r}
# Calculate the total number of forecasting horizon needed for ARIMA
forecast_horizon_arima_evaluation <- nrow(ts_validation) + nrow(ts_evaluation) 

# Forecast and report 80% & 90% prediction intervals
fc_arima_evaluation <- fit_arima %>%
  forecast(h = forecast_horizon_arima_evaluation , level = c(80, 95))

# Calculate accuracy for the validation predictions
accuracy_arima_evaluation <- accuracy(fc_arima_evaluation, ts_evaluation)
accuracy_arima_evaluation

# Plot the predictions
autoplot(ts_train, sales) +
  autolayer(fc_arima_evaluation) +
  labs(title = "ARIMA Forecast (28 days)", x = "Date", y = "Sales")
...
```{r}
fc_ets_eval <- fit_ets %>%
  forecast(new_data = ts_evaluation, level = c(80, 95))

accuracy_ets_evaluation <- accuracy(fc_ets_eval, ts_evaluation)
accuracy_ets_evaluation
```
...
In order to create features for light gbm model - we need to prepare
full_dataset to handle the feature enginerreing from 'LightGBM.Rmd'
notebook
...
# create splits
dt_tr <- as.data.table(full_dataset)
dt_tr[, split := "train"]

dt_va <- as.data.table(full_dataset_validation)
dt_va[, split := "val"]

dt_te <- as.data.table(full_dataset_evaluation)
dt_te[, split := "test"]

# bind tables to create features
dt <- rbindlist(list(dt_tr, dt_va, dt_te), use.names = TRUE, fill = TRUE)
...
dt[, roll_med_28  := frollapply(sales, 28, median, align = "right"), by = id]
...
-   zero_ratio_28: Fraction of zero-sales days in last 28 days
    (Distinguishes slow-moving items from consistently selling ones)
-   sell_price_ff: Current effective price
-   price_rel: Current price relative to item’s average price (Is the
    item expensive or discounted relative to itself)
...
feat_cols <- c(
  ...
  "roll_mean_7","roll_mean_28","roll_med_28","roll_sd_28","zero_ratio_28",
...
train_1 <- dt[split == "train" & !is.na(target_1)]
val_1   <- dt[split == "val"   & !is.na(target_1)]
test_1  <- dt[split == "test"  & !is.na(target_1)]

train_1 <- train_1[complete.cases(train_1[, ..feat_cols])]
val_1   <- val_1[complete.cases(val_1[, ..feat_cols])]
test_1  <- test_1[complete.cases(test_1[, ..feat_cols])]
...
y_te <- log1p(test_1$target_1)

# construct feature matrix for the test set
X_te <- as.matrix(test_1[, ..feat_cols])

pred_te <- expm1(predict(m1, X_te))
y_te_raw <- test_1$target_1

# compute residuals 
e_te <- y_te_raw - pred_te

lgbm_rmse_test <- sqrt(mean(e_te^2, na.rm = TRUE))
lgbm_rmse_test
...
Run cross validation. Note: At the moment commented out for knitting
efficiency. The output of the cross validaiton is saved in csv file and
read in the code at the bottom of the next chunk.
...
```
The best model is identified as follows - learning rate: 0.05 - num
leaves: 64 - feature fraction: 0.9 - bagging fraction: 0.9 -
min_data_in_leaf: 20

Let's test the model with such settings on validation test

### 5. Validate the predictions and see if predictions improved and analyse the residuals
...
m1_tuned <- lgb.train(
  ...
  data    = dtrain_tuned,
  nrounds = 5000,         
  valids  = list(train = dtrain_tuned, val = dval_tuned), #BUGRA: "train = dtrain" is added to keep RMSE for train in each interation for fitting graph
  early_stopping_rounds = 100,
  verbose = 1,
  record = TRUE # BUGRA: Store the evaluation results per iteration for fitting graph
)
...
Extract train and validation loss to observe whether under- or
over-fitting occurred.

```{r}
# Extract learning curves (i.e., RMSE per iteration)
train_rmse_tuned <- unlist(m1_tuned$record_evals$train$rmse$eval)
val_rmse_tuned   <- unlist(m1_tuned$record_evals$val$rmse$eval)

# create a new data frame that has all the data for plotting learning curve
learning_curve <- data.frame(
  iter = seq_along(train_rmse_tuned),
  rmse_train = train_rmse_tuned,
  rmse_val   = val_rmse_tuned
)

# Plot the learning curve.It has: iter, rmse_train, rmse_val
plot(
  learning_curve$iter,
  learning_curve$rmse_train,
  type = "l",
  col = "blue",
  lwd  = 2,
  xlab = "Iteration",
  ylab = "RMSE (log1p scale)",
  main = "LightGBM tuned model: training vs validation loss"
)

lines(
  learning_curve$iter,
  learning_curve$rmse_val,
  col = "red",
  lwd = 2
)
...
legend(
  "topright",
  legend = c("Training RMSE", "Validation RMSE"),
  lty = 1,
  col = c("blue", "red"),
  lwd = 2,
  bty = "n"
...
## Generate aggregated Portmanteau tests for all of the models (table view for convenience)
```{r}
ljung_box_results <- dplyr::bind_rows(
  list(
    Mean = models |>
      select(Mean) |>
      augment() |>
      features(.innov, ljung_box, lag = 28, dof = 0),
    ...
    LightGBM = daily_train_residual_ts |>
      features(average_residual, ljung_box, lag = 28, dof = 0),

    LightGBM_tuned = daily_train_residual_ts_tuned |>
      features(average_residual_tuned, ljung_box, lag = 28, dof = 0)
  ),
  .id = ".model"
)
...
## 28 day forecast plot for Tuned LightGBM
```{r}
library(data.table)
library(ggplot2)
...
origin_date <- max(dt[split %in% c("train","val"), date], na.rm = TRUE)
h <- 28
future_dates <- seq(origin_date + 1, origin_date + h, by = "day")

# keep the last 28 observed days per id
hist <- dt[split %in% c("train","val") & date <= origin_date,
           .(id, date, sales, wday, month, year, snap_TX, sell_price_ff)]
hist <- hist[order(id, date)]
hist <- hist[, tail(.SD, 28), by = id]
hist[, id := as.character(id)]

# take the rows from test for the next 28 days
fut <- dt[split == "test" & date %in% future_dates,
          .(id, date, wday, month, year, snap_TX, sell_price_ff)]
fut[, id := as.character(id)]
fut <- fut[order(id, date)]

stopifnot(length(unique(fut$date)) == h)

# price fallback if any NA sneaks in
fut[, sell_price_ff := nafill(sell_price_ff, type = "locf"), by = id]
last_price <- hist[, .(last_p = tail(sell_price_ff, 1)), by = id]
fut <- merge(fut, last_price, by="id", all.x=TRUE)
fut[is.na(sell_price_ff), sell_price_ff := last_p]
fut[, last_p := NULL]

# precompute item mean price from available history
price_mean_item <- dt[, .(price_mean_item = mean(sell_price_ff, na.rm=TRUE)), by = id]
fut <- merge(fut, price_mean_item, by="id", all.x=TRUE)
fut[is.na(price_mean_item), price_mean_item := mean(dt$sell_price_ff, na.rm=TRUE)]

# predictions
fut[, pred := NA_real_]

# rolling 28-day sales 
buf <- hist[, .(sales_buf = list(sales)), by = id] 
buf[, id := as.character(id)]

roll_feats <- function(x28) {
  lag_1  <- tail(x28, 1)
  lag_2  <- tail(x28, 2)[1]
  lag_3  <- tail(x28, 3)[1]
  lag_7  <- tail(x28, 7)[1]
  lag_14 <- tail(x28, 14)[1]
  lag_28 <- x28[1]

  last7 <- tail(x28, 7)

  roll_mean_7  <- mean(last7)
  roll_mean_28 <- mean(x28)
  roll_med_28  <- median(x28)
  roll_sd_28   <- sd(x28)

  nz_28 <- sum(x28 > 0)
  zero_ratio_28 <- 1 - nz_28 / 28
  ...
  list(
    ...
    roll_med_28 = roll_med_28,
    roll_sd_28 = roll_sd_28,
    zero_ratio_28 = zero_ratio_28
  )
}

# price series to compute price_lag_7
price_series <- dt[date >= (origin_date - 7) & date <= (origin_date + h),
                   .(id, date, sell_price_ff)]
price_series[, id := as.character(id)]
setorder(price_series, id, date)

setkey(fut, id, date)

for (d in future_dates) {

  todays <- fut[.(unique(fut$id), d), nomatch = 0L]  
  todays <- merge(todays, buf, by="id", all.x=TRUE)

  # price lag 7 
  p_lag <- price_series[date == (d - 7), .(id, price_lag_7 = sell_price_ff)]
  todays <- merge(todays, p_lag, by="id", all.x=TRUE)
  todays[is.na(price_lag_7), price_lag_7 := sell_price_ff]

  feat_dt <- todays[, {
    x28 <- sales_buf[[1]]
    if (length(x28) < 28) x28 <- c(rep(0, 28 - length(x28)), x28)

    rf <- roll_feats(x28)

    price_pct_change_7 <- ifelse(price_lag_7 > 0, (sell_price_ff - price_lag_7) / price_lag_7, 0.0)
    is_price_drop <- as.integer(price_pct_change_7 < 0)
    price_rel <- sell_price_ff / price_mean_item

    list(
      id = id, date = date,
      wday = wday, month = month, year = year, snap_TX = snap_TX,
      lag_1 = rf$lag_1, lag_2 = rf$lag_2, lag_3 = rf$lag_3,
      lag_7 = rf$lag_7, lag_14 = rf$lag_14, lag_28 = rf$lag_28,
      roll_mean_7 = rf$roll_mean_7,
      roll_mean_28 = rf$roll_mean_28,
      roll_med_28 = rf$roll_med_28,
      roll_sd_28 = rf$roll_sd_28,
      zero_ratio_28 = rf$zero_ratio_28,
      sell_price_ff = sell_price_ff,
      price_rel = price_rel,
      price_pct_change_7 = price_pct_change_7,
      is_price_drop = is_price_drop
    )
  }, by = id]

  X <- as.matrix(feat_dt[, ..feat_cols])
  pred_vec <- expm1(predict(m1_tuned, X))
  pred_vec[pred_vec < 0] <- 0

  pred_dt <- data.table(id = feat_dt$id, date = feat_dt$date, pred = pred_vec)
  setkey(pred_dt, id, date)
  fut[pred_dt, pred := i.pred]

  upd <- pred_dt[, .(id, pred)]
  setkey(upd, id)

  buf <- merge(buf, upd, by="id", all.x=TRUE)
  buf[, sales_buf := lapply(seq_len(.N), function(i) {
    x28 <- sales_buf[[i]]
    if (length(x28) < 28) x28 <- c(rep(0, 28 - length(x28)), x28)
    c(tail(x28, 27), pred[i])
  })]
  buf[, pred := NULL]
}

# aggregate historic data and forecast
hist_total <- dt[split %in% c("train","val") & date <= origin_date,
                 .(total_sales = sum(sales, na.rm=TRUE)), by = date]

fc_total <- fut[, .(total_sales = sum(pred, na.rm=TRUE)), by = date]

hist_total[, series := "Historic Data"]
fc_total[, series := "Forecast (28 days)"]

plot_dt <- rbindlist(list(hist_total, fc_total), use.names=TRUE, fill=TRUE)
setorder(plot_dt, date)

ggplot(plot_dt, aes(x = date, y = total_sales, color = series)) +
  geom_line(linewidth = 0.8) +
  scale_color_manual(values = c("Historic Data" = "black", "Forecast (28 days)" = "blue")) +
  labs(
    title = "Tuned LGBM Forecast (28 days)",
    x = "Date",
    y = "Sales",
    color = ""
  ) +
